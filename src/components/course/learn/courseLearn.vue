        ...
        <div class="es-wrap">
            <div class="lesson-dashboard lesson-dashboard-open" id="lesson-dashboard" data-course-id="39" data-course-uri="/course/39" data-dashboard-uri="/course/39/learn" data-watch-limit="" data-starttime="" data-hide-media-lesson-learn-btn="" data-widget-cid="widget-0">
                <div class="dashboard-content">
                    <span class="btn btn-primary  nav-btn back-course-btn" @click="comingclicked($route.query.cid)"><span class="glyphicon glyphicon-chevron-left"></span> 返回课程</span>
                    ...
                    <div class="dashboard-body">
                        <div class="lesson-content" id="lesson-video-content" data-role="lesson-content" style="" data-user-id="1" data-balloon-player="1">
                            <video id="my-video" :src="videoSrc" :poster="videoImg" :muted="muteStatus" ontimeupdate autoplay="playStatus" width="100%" controls="true" controlslist="nodownload" preload="auto">
                                <p class="vjs-no-js">
                    ...
                    <div class="toolbar-nav tabbable">
                        ...
                        <ul class="toolbar-nav-stacked" id="lesson-toolbar-secondary">
                            <li class="hide-pane" style="">
                                <a href="javascript:" @click="hideTab"><span class="glyphicon glyphicon-chevron-right"></span></a>
                            </li>
                        </ul>
                    </div>
                                ...
                                <div data-pane="lesson" class="lesson-pane" data-widget-cid="widget-2" style="display: block;">
                                    <div class="course-item-list-in-toolbar-pane ps-container">
                                        ...
                                        <ul class="period-list" id="course-item-list">
                                            <li class="period lesson-item lesson-item-262" data-id="262" data-num="1" style="cursor:pointer" v-for="(item ,completeV) in stuVideoArr" :key="completeV" @click="learnDetail(item.cId,courseLearnId)">
                                            ...
                                            <li class="period lesson-item lesson-item-263" data-id="263" data-num="2" v-for="(item ,completeT) in stuTestArr" style="cursor:pointer" :key="completeT" @click="startTestDetail(items.cId,courseLearnId)">
                                        ...
                                        <!-- 正在学习 -->
                                        <ul class="period-list" id="course-item-list">
                                            <li class="period lesson-item lesson-item-262" style="cursor:pointer"  data-id="262" data-num="1" v-for="(learn,index)  in lVideoArr" :key="learn.cId" @click="learnDetail(learn.cId,courseLearnId)">
                                            ...
                                            <li class="period lesson-item lesson-item-263" data-id="263" data-num="2" v-for="(learnTest,doingT) in lTestArr" :key="doingT"  @click="startTestDetail(items.cId,courseLearnId)" style="cursor:pointer" >
                                                    <i class="es-icon es-icon-doing color-primary status-icon"></i>
                                                    <span class="title"> 课时{{index+++1}}：{{learnTest.name}}</span>
                                                    <span class="course-type">
                                                        <i class="es-icon es-icon-check" data-toggle="tooltip" data-placement="top" title="" data-original-title="试卷"></i>
                                                    </span>
                                        ...
                                        <ul class="period-list" id="course-item-list">
                                            <li class="period lesson-item lesson-item-262" data-id="262" data-num="1" v-for="(itema,videoI) in unVideoArr" :key="videoI">
                                                <div v-if="itema.cId === nextLearn.cId" @click="learnDetail(itema.cId,courseLearnId)"  style="cursor:pointer" class="course-lesson">
                                            ...
                                            <li class="period lesson-item lesson-item-263" data-id="263" data-num="2" v-for="(items,index) in unTestArr" :key="items.cId" >
                                                <div  v-if="items.cId === nextLearn.cId" @click="startTestDetail(items.cId,courseLearnId)" style="cursor:pointer" class="course-lesson" >
                                                    <i class="es-icon es-icon-undone color-primary status-icon"></i>
                                                    <span class="title"> 课时{{index+++1}}：{{items.name}}</span>
                                                    <span class="course-type">
                                                        <i class="es-icon es-icon-check" data-toggle="tooltip" data-placement="top" title="" data-original-title="试卷"></i>
                                                    </span>
                                                </div>
                                        ...
                                        <!-- <button class="btn btn-primary btn-lg" data-toggle="modal" data-target="#course-learned-modal">
                	开始演示模态框
                </button> -->
            ...
            <!-- /.modal -->
            <div class="modal" id="x" style="display:none;">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
...
<script>
    ...
    import {videojs} from '../../../../static/assets/libs/video/video.js'
    ...
    export default {
        ...
        data() {
            return {
                videoSrc: "",
                _dom: "",
                videoImg: 'https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=1242428255,3195206803&fm=26&gp=0.jpg',
                playStatus: '',
                muteStatus: '',
                isMute: true,
                isPlay: false,
                ...
                courseLearnId: this.$route.query.cid, //课时id,
                isClassFinish: false, //课程完成
                communicationArr: [],
                fileArr: [],
                nextLearn:null,
            }
        ...
        // beforeCreate() {
        //     this.courseLearnId = this.$route.query.id
        // },
        created() {
        ...
        methods: {
            ...
            startLearn(){
                start(this.$route.query.cid, this.$route.query.id, this.userToken).then(response => {
                    console.log('开始学习',this.$route.query.id, this.userToken)
                })
            },
            //video进度时间监听
            // videoFun(id,userToken) {
            //     var videoPlayer = document.getElementById("my-video");
            //     var time = parseInt(this.videoTotal) * 0.5 * 60;
            //     var iscomplete = 0
            //     videoPlayer.ontimeupdate = function() {
            //         console.log("this.videoTotal",time)
            //         console.log('tag', videoPlayer.currentTime) ;
            //         if (videoPlayer.currentTime > time && iscomplete == 0) {
            //             complete(id, userToken).then(response => { 
            //                 console.log("此课时已完成学习")
            //                 iscomplete = 1
            ...
            getVideo() {
                queryClassById(this.$route.query.id, this.userToken).then(response => {
                    this.videoSrc = response.data.data.videoPath
                    this.videoTotal = response.data.data.duration
                var videoPlayer = document.getElementById("my-video");
                var time = parseInt(this.videoTotal) * 0.8 * 60;
                var iscomplete = 0
                ...
                videoPlayer.ontimeupdate = function() {
                    ...
                    if (videoPlayer.currentTime > time && iscomplete == 0) {
                        complete(id, userToken).then(response => { 
                            console.log("此课时已完成学习")
                            iscomplete = 1
                            if (_self.courseclearningList.length>0){
                            _self.nextLearn = _self.coursecunlearnedList[0]
                            }
                            else if(_self.coursecunlearnedList.length>0){
                            _self.nextLearn = _self.coursecunlearnedList[1]
                            }
                            console.log('xiayigeid', _self.nextLearn.cId)
                        })
                    }
                };
                })
